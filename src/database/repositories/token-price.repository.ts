...
export class TokenPriceRepository extends BaseRepository<TokenPrice> {
  ...
  public async getRateETHToMina() {
    ...
    .select(`(select CAST(price_usd AS REAL) from token_prices tp2 where tp2.symbol = 'ETH') / (select CAST(price_usd AS REAL) from token_prices tp where tp.symbol = 'MINA') as rateETHMINA`)
    .getRawOne()
